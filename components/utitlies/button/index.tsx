...
const Button: React.FC<ButtonProps> = ({ bg, isNormal, color="text-textblack", isActive = false, isOpen = false, lgH = false, imagePath, showRightIcon = false, text }) => {
    console.log(isActive)
    return (
        <button
            className={`flex w-full items-center ${isNormal && !isActive ? bg : !isNormal && isActive ? bg : "bg-transparent"}  ${isOpen ? "justify-between" : "justify-center"} px-4 ${lgH ? "h-[3.8rem]" : "h-[2.8rem]"} ${lgH ? "rounded-[2rem]" : "rounded-3xl"}`}
        >
            <div className={`flex items-center ${isOpen ? "justify-start" : "justify-center"} w-full gap-2`}>
                {imagePath && (
                    <>
                        {
                            lgH ?
                ...
                {isOpen && <div className={` ${isNormal && !isActive ?color : !isNormal && isActive ? color : color}     leading-5 text-sm font-[300] font-circular`}>{text}</div>}
            </div>
